function tracker = create_csr_tracker(img, init_bbox, init_params)
    ...
    sigma = ((sqrt(prod(base_target_sz)) * rescale_ratio) / cell_size) * (1/12);
    Y = fft2(gaussian_shaped_labels(1,sigma, floor(rescale_template_size([2,1]) / cell_size)));

    %store pre-computed cosine window
    if strcmp(init_params.window_type, 'hann')
        window = hann(size(Y,1)) * hann(size(Y,2))';
    elseif strcmp(init_params.window_type, 'hamming')
        window = hamming(size(Y,1)) * hamming(size(Y,2))';
    elseif strcmp(init_params.window_type, 'kaiser')
        alpha = init_params.window_param; 
        window = kaiser(size(Y,1), alpha) * kaiser(size(Y,2), alpha)';
    else
        error('Unknown tracker window type.');
    end
    ...
    f = get_csr_features(img, c, currentScaleFactor, template_size, ...
        rescale_template_size, window, feature_type, w2c, cell_size, use_mex);
    ...
    tracker.estimate_scale = init_params.estimate_scale;
    if init_params.estimate_scale
        tracker.ysf = ysf;
    ...
    tracker.mask = single(mask);
    tracker.use_mex = use_mex;
    
    tracker.frame = 1;
    
    tracker.update_background = init_params.update_background;
    tracker.f = f;
    
    if init_params.update_background
        lr_background = init_params.lr_background;
        lr_foreground = init_params.lr_foreground;
        K = get_interp_kernel(template_size, base_target_sz, ...
            init_params.background_sigma_factor);
        K = mexResize(K, [size(f,1), size(f,2)], 'auto');
        K = K / max(K(:));
        K = K * (lr_background - lr_foreground) + (1 - lr_background);
        tracker.inter_kernel = K;
    end
    
end  % endfunction


function K = get_interp_kernel(template_sz, target_sz, sigma_factor)

    [Y, X] = ndgrid(floor(1:template_sz(2)) - template_sz(2) / 2, ...
        floor(1:template_sz(1)) - template_sz(1) / 2);
    sigma_x = sigma_factor * target_sz(1);
    sigma_y = sigma_factor * target_sz(2);
    
    K = exp(-0.5 * (((X.^2) / (2*sigma_x^2)) + ((Y.^2) / (2*sigma_y^2))) );
